...
{
    [Authorize]
    public class LedgerHeader_WithCalculationController : System.Web.Mvc.Controller
    {
        ...
        public LedgerHeader_WithCalculationController(ILedgerHeaderService LedgerHeaderService, IUnitOfWork unitOfWork, IExceptionHandlingService exec)
        {
        ...
        public ActionResult Create(int id)
        {
            ...
            if (settings != null)
            {
                if (settings.CalculationId != null)
                {
                    ...
                    {
                        return RedirectToAction("Create", "LedgerSettings", new { id = id }).Warning("Ledger posting settings is not defined for current site and division.");
                    }
        ...
        public ActionResult DeleteConfirmed(ReasonViewModel vm)
        {
            ...
            if (ModelState.IsValid && BeforeSave && !EventException)
            {
                ...
                foreach (var item in LedgerLines)
                {
                    ...
                    var LineCharges = (from p in db.LedgerLineCharge
                                       where p.LineTableId == item.LedgerLineId 
                                       select p).ToList();
                    foreach (var item1 in LineCharges)
                    {
                ...
                var HeaderCharges = (from p in db.LedgerHeaderCharges
                ...
                {
                    item2.ObjectState = Model.ObjectState.Deleted;
                    db.LedgerHeaderCharges.Remove(item2);
                }
