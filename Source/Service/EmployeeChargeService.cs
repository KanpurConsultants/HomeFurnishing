...
namespace Service
{
    public interface IEmployeeChargeService : IDisposable
    {
        EmployeeCharge Create(EmployeeCharge pt);
        void Delete(int id);
        void Delete(EmployeeCharge pt);
        EmployeeCharge Find(int id);
        IEnumerable<EmployeeCharge> GetPagedList(int pageNumber, int pageSize, out int totalRecords);
        void Update(EmployeeCharge pt);
        EmployeeCharge Add(EmployeeCharge pt);
        IEnumerable<EmployeeCharge> GetEmployeeChargeList();
        Task<IEquatable<EmployeeCharge>> GetAsync();
        Task<EmployeeCharge> FindAsync(int id);
        int NextId(int id);
        int PrevId(int id);
        IEnumerable<HeaderChargeViewModel> GetCalculationFooterList(int HeaderTableId);
    }

    public class EmployeeChargeService : IEmployeeChargeService
    {
        ApplicationDbContext db = new ApplicationDbContext();
        private readonly IUnitOfWorkForService _unitOfWork;
        public EmployeeChargeService(IUnitOfWorkForService unitOfWork)
        {
        ...
        {
            var temp=from p in db.EmployeeCharge
        ...
        public EmployeeCharge Find(int id)
        {
            return (_unitOfWork.Repository<EmployeeCharge>().Find(id));
        }

        public EmployeeCharge Create(EmployeeCharge pt)
        {
            pt.ObjectState = ObjectState.Added;
            _unitOfWork.Repository<EmployeeCharge>().Insert(pt);
        ...
        {
            _unitOfWork.Repository<EmployeeCharge>().Delete(id);
        }

        public void Delete(EmployeeCharge pt)
        {
            _unitOfWork.Repository<EmployeeCharge>().Delete(pt);
        }

        public void Update(EmployeeCharge pt)
        {
            pt.ObjectState = ObjectState.Modified;
            _unitOfWork.Repository<EmployeeCharge>().Update(pt);
        }

        public IEnumerable<EmployeeCharge> GetPagedList(int pageNumber, int pageSize, out int totalRecords)
        {
            var so = _unitOfWork.Repository<EmployeeCharge>()
                .Query()               
        ...
        public IEnumerable<EmployeeCharge> GetEmployeeChargeList()
        {
            var pt = _unitOfWork.Repository<EmployeeCharge>().Query().Get().OrderBy(m=>m.HeaderTableId);

            return pt;
        }

        public EmployeeCharge Add(EmployeeCharge pt)
        {
            _unitOfWork.Repository<EmployeeCharge>().Insert(pt);
            return pt;
        }
        ...
        public int NextId(int id)
        {
            ...
            if (id != 0)
            {
                temp = (from p in db.EmployeeCharge
        ...
        public Task<IEquatable<EmployeeCharge>> GetAsync()
        {
            throw new NotImplementedException();
        }

        public Task<EmployeeCharge> FindAsync(int id)
        {
            throw new NotImplementedException();
        }
    }
