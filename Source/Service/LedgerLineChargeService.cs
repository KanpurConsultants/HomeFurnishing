...
namespace Service
{
    public interface ILedgerLineChargeService : IDisposable
    {
        LedgerLineCharge Create(LedgerLineCharge pt);
        void Delete(int id);
        void Delete(LedgerLineCharge pt);
        LedgerLineCharge Find(int id);
        IEnumerable<LedgerLineCharge> GetPagedList(int pageNumber, int pageSize, out int totalRecords);
        void Update(LedgerLineCharge pt);
        LedgerLineCharge Add(LedgerLineCharge pt);
        IEnumerable<LedgerLineCharge> GetLedgerLineChargeList();

        // IEnumerable<LedgerLineCharge> GetLedgerLineChargeList(int buyerId);
        Task<IEquatable<LedgerLineCharge>> GetAsync();
        Task<LedgerLineCharge> FindAsync(int id);
    ...
    public class LedgerLineChargeService : ILedgerLineChargeService
    {
        ApplicationDbContext db = new ApplicationDbContext();
        private readonly IUnitOfWorkForService _unitOfWork;
        private readonly Repository<LedgerLineCharge> _LedgerLineChargeRepository;
        RepositoryQuery<LedgerLineCharge> LedgerLineChargeRepository;
        public LedgerLineChargeService(IUnitOfWorkForService unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }


        public LedgerLineCharge Find(int id)
        {
            return (_unitOfWork.Repository<LedgerLineCharge>().Find(id));
        }

        public LedgerLineCharge Create(LedgerLineCharge pt)
        {
            pt.ObjectState = ObjectState.Added;
            _unitOfWork.Repository<LedgerLineCharge>().Insert(pt);

            return pt;
        }
        public IEnumerable<LineChargeViewModel> GetCalculationProductList(int LineTableId)
        {
            return (from p in db.LedgerLineCharge
        ...
        {
            //var temp=from p in db.LedgerLineCharge
            //        where p.LineTableId == (from p1 in db.LedgerLine
            //                                orderby p1.LedgerLineId descending
            //                                    where p1.LedgerHeaderId==HeaderId
            //                                    select p1.LedgerLineId
            //                                    ).FirstOrDefault()
            //        orderby p.Sr
            //        select new LineChargeViewModel
            //        {
        ...
        {
            _unitOfWork.Repository<LedgerLineCharge>().Delete(id);
        }

        public void Delete(LedgerLineCharge pt)
        {
            _unitOfWork.Repository<LedgerLineCharge>().Delete(pt);
        }

        public void Update(LedgerLineCharge pt)
        {
            pt.ObjectState = ObjectState.Modified;
            _unitOfWork.Repository<LedgerLineCharge>().Update(pt);
        }

        public IEnumerable<LedgerLineCharge> GetPagedList(int pageNumber, int pageSize, out int totalRecords)
        {
            var so = _unitOfWork.Repository<LedgerLineCharge>()
                .Query()               
        ...
        public IEnumerable<LedgerLineCharge> GetLedgerLineChargeList()
        {
            var pt = _unitOfWork.Repository<LedgerLineCharge>().Query().Get().OrderBy(m=>m.LineTableId);

            return pt;
        }

        public LedgerLineCharge Add(LedgerLineCharge pt)
        {
            _unitOfWork.Repository<LedgerLineCharge>().Insert(pt);
            return pt;
        }
        ...
        public int NextId(int id)
        {
            ...
            if (id != 0)
            {
                temp = (from p in db.LedgerLineCharge
        ...
        public Task<IEquatable<LedgerLineCharge>> GetAsync()
        {
            throw new NotImplementedException();
        }

        public Task<LedgerLineCharge> FindAsync(int id)
        {
            throw new NotImplementedException();
        }
    }
